...
//TODO Realize getting and transfer data by special workers.
//TODO Improve MVP pattern for all UI elements.


public class AddItemActivity extends AppCompatActivity implements View.OnClickListener, AddItemContract.View {
    ...
    private void init() {
        ...
        presenter = new AddItemPresenter(this, getApplicationContext());
    ...
    public void showToast(int messageResId) {
        ...
        Toast.makeText(AddItemActivity.this, getString(messageResId), Toast.LENGTH_LONG).show();
    }
    ...
    public ExcelData getData() {
        ...
        } else if (category.contains("-") & (category.length() < 2)) //проверка на выбор этажа
        {
            ...
            return null;
        } else if (name.length() == 0 || roomNumber.length() == 0 || need.length() == 0 || responsible.length() == 0) //проверка на заполненость обязательных полей.
        {
